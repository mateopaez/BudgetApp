...
**Current state:** All Firestore fields are plaintext. Admins with DB access can read everything.

**Goal:** Encrypt sensitive fields in the browser so Firestore only stores ciphertext; keep `postedAt` (and maybe `accountId`, `kind`) queryable.

### 11a. Key management

- [ ] Derive encryption key from user password (or separate vault password) using PBKDF2/Argon2 + salt stored in Firestore
- [ ] Or: random data key wrapped by key derived from password; store wrapped key in Firestore
- [ ] On login: derive key, cache in memory only (never localStorage); clear on sign-out
- [ ] **Recovery story:** losing password = losing data — document clearly; optional recovery key export

### 11b. Encrypt / decrypt layer

- [ ] Create `EncryptionService` with `encrypt(plaintext)` / `decrypt(ciphertext)` using Web Crypto AES-GCM
- [ ] Define per-document **encrypted blob** field vs cleartext index fields:
  - Transactions: encrypt `merchant`, `description`, `amount`, `categoryId`, `split`; keep `postedAt`, `accountId`, `kind` plain for queries
  - Accounts: encrypt `name`, `openingBalance`; keep `type`, `openingDate`?
  - Categories: encrypt `name` (or hash for lookup)
- [ ] Wrap existing services (`transaction.service.ts`, etc.) to encrypt on write, decrypt on read

### 11c. Query limitations & migration

- [ ] Accept that **text search on merchant** won’t work server-side — filter client-side after decrypt, or maintain blind index (advanced)
- [ ] Import hash computation must use plaintext before encrypt or use deterministic hash of normalized fields
- [ ] One-time migration script: decrypt N/A for existing users — encrypt on next write or batch migration tool
- [ ] Update Firestore rules (optional): validate field shapes without needing plaintext

**Files:** new `encryption.service.ts`, all `*.service.ts` in `core/services/`, `auth.service.ts`

**Risk:** High complexity, breaks simple debugging, password reset becomes hard. **Recommend shipping §1–§8 first.**

**Acceptance:** Inspecting a transaction document in Firebase Console shows encrypted blob for merchant/amount; app displays correct values when logged in.
...
| **G — Privacy** | 11 | Large architectural change; do last |
